#!/usr/bin/env python3
"""One-off: import the second trap batch — 6 files, 3 duplicate-title pairs
renamed distinct same as every prior batch.

    python3 scripts/import_trap_batch2.py
"""
...
TRACKS = [
    ("Chrome Habit_trap.mp3", "trap", "Chrome Habit"),
    ("Chrome Habit_1_trap.mp3", "trap", "Static Habit"),
    ("Chrome Pulse_trap.mp3", "trap", "Chrome Pulse"),
    ("Chrome Pulse_1_trap.mp3", "trap", "Neon Pulse Drift"),
    ("Velvet Rim_trap.mp3", "trap", "Velvet Rim"),
    ("Velvet Rim_1_trap.mp3", "trap", "Low Rim Static"),
]
...
def main() -> None:
    ...
    for fname, show, title in TRACKS:
        ...
        print(f"  {show:<12} {title:<20} bpm={bpm:5.1f}  -> {web_name}  ({mb:.1f} MB)")
